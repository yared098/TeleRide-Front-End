...
const GOOGLE_MAPS_API_KEY = import.meta.env.VITE_GOOGLE_MAPS_API_KEY;
const containerStyle = { width: "100%", height: "100%" };
const FALLBACK_POSITION = { lat: 9.03, lng: 38.74 }; // Addis Ababa fallback
...
const RideRequest = () => {
    ...
    // Request Telegram Mini App location first, fallback to browser geolocation
    useEffect(() => {
        const getLocation = async () => {
            // Telegram WebApp location
            if (window.Telegram?.WebApp) {
                try {
                    await window.Telegram.WebApp.enableGeolocation(); // Request location
                    navigator.geolocation.getCurrentPosition(
                        (pos) => setPosition({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
                        (err) => {
                            console.warn("Telegram geolocation denied, using fallback:", err);
                            setPosition(FALLBACK_POSITION);
                        }
                    );
                    return;
                } catch (err) {
                    console.warn("Telegram geolocation not available, fallback:", err);
                }
            }
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (pos) => setPosition({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
                    (err) => {
                        console.warn("Browser geolocation denied, using fallback:", err);
                        setPosition(FALLBACK_POSITION);
                    }
                );
            } else {
    ...
    return (
        <div className="h-screen w-full relative bg-gray-100">
            {/* Map and UI code stays the same */}
        </div>
    );
};
