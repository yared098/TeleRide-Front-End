...
const RideRequest = () => {
  const { user, token } = useAuth();
  const { connected, requestRide } = useRideSocket(
    token,
    user,
    () => {},
    () => {},
  ...
  return (
    <div className="h-screen w-full relative bg-gray-100">
      ...
      {message && (
        <div
          className={`absolute top-20 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-xl text-center shadow-md ${
      ...
      {/* 🔌 Socket Status */}
      <div
        className={`absolute bottom-2 left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full text-sm font-medium shadow-md ${
